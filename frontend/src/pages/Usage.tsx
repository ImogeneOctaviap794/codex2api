...
function StatusCodeBadge({ log }: { log: UsageLog }) {
  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <span tabIndex={-1} aria-label={`${log.status_code} ${message}`} className="inline-flex">
          {badge}
        </span>
      </TooltipTrigger>
...
export default function Usage() {
  ...
  const buildLogBaseFilterParams = useCallback(() => {
    const { start, end } = resolveRangeISO(timeRange, customRange)
    return {
      start,
      end,
  ...
  }, [timeRange, customRange, searchQuery, filterModel, filterEndpoint, filterApiKeyId, filterAccountId, filterFast, filterType, channel, filterErrorKind, filterRetry, filterTransport])

  const buildLogFilterParams = useCallback(() => ({
    ...buildLogBaseFilterParams(),
    status: filterStatus && filterStatus !== 'error' ? filterStatus : undefined,
    errorOnly: filterStatus === 'error' ? 'true' : undefined,
  }), [buildLogBaseFilterParams, filterStatus])
  ...
  const loadErrorSummary = useCallback(async () => {
    ...
    try {
      const summary = await api.getUsageLogsErrorSummary(buildLogBaseFilterParams())
      if (requestID !== summaryRequestRef.current) return
      setErrorSummary(summary)
    } catch {
      if (requestID === summaryRequestRef.current) setErrorSummary(null)
    }
  }, [buildLogBaseFilterParams])
